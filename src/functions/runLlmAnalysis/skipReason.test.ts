...
function makeEnriched(overrides: Partial<EnrichedTicker>): EnrichedTicker {
  return {
    ...
    rawOptions: { symbol: 'TEST', ivRank: 45, ivPercentile: 0, iv30d: 0, hv30d: 0, volSurface: [], candidateStrikes: [], fetchedAt: '' },
    ...overrides,
...
describe('buildSkipReason', () => {
  ...
  it('earnings reason takes priority over IV rank', () => {
    const enriched = makeEnriched({
      earningsInWindow: true,
      ivRankSignal: 'SKIP',
      rawFundamentals: { symbol: 'AMZN', earningsDte: 4, earningsDate: '2026-04-29', fetchedAt: '' },
      rawOptions: { symbol: 'AMZN', ivRank: 20, ivPercentile: 0, iv30d: 0, hv30d: 0, volSurface: [], candidateStrikes: [], fetchedAt: '' },
    });
